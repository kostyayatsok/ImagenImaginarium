...
async def get_media(text):
    ...
    emb_true = await image_generation.text_embedding(text)
    print("embedding getted")
    image_true = await image_generation.generate_image(emb_true)
    print("first picture generated")
    image_true.save("0.png")
    media.append("0.png")
    for i in range(2):
        new_text = await edit_text_bert(text, 1)
        emb = await image_generation.text_embedding(new_text)
        image = await image_generation.generate_image(emb)
        image.save(str(i + 1) + ".png")
        media.append(str(i + 1) + ".png")
    for i in range(2):
        emb = await image_generation.text_embedding(text)
        emb = await edit_text_latent(emb, 7)
        image = await image_generation.generate_image(emb)
